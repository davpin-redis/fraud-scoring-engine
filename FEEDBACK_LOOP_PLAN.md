...
- **Phase 5 — Large scale (GCP):** D3-large, D4-large/Gatling, F1. ◑ **code + runbook delivered** — the slow-loop **`retrain_loop.py`** (audit Parquet + matured labels → train → export ONNX → `model:invalidate`) closes the production loop and is CI-proven (30 pipeline tests); the large-scale run is documented in `LOAD_TEST_RUNBOOK.md` (collapsed-store seed, pipeline services, engines with `model.type=onnx`, retrain cadence, sizing). GCP-side items now done in code: the engine feature snapshot exposes all 14 model features (incl. derived `new_payee`, verified by `HotWindowRuleIT`); the Gatling `FeedbackLoopSimulation` + `chargeback_feed.py` label feed are provided (Gatling compiles, feed unit-tested). **Only remaining is deployment config:** a shared/GCS `fraud.model.path` all engines read. *([you] run the GCP execution)*
